  ...
  describe('Login', () => {
    ...
    describe('When logged in', () => {
      ...
      test('a new blog can be created', async ({ page }) => {
        ...
        await expect(
          page.getByText(`\`${title}\` by ${author} added!`)
        ).toBeVisible();
      ...
      describe('When some blogs saved', () => {
        ...
        test('blog link navigates to blog page', async ({ page }) => {
          ...
          await expect(
            page.getByText('Marvel: Avengers Doomsday')
          ).toBeVisible();
          await expect(page.getByText('www.coming.com')).toBeVisible();
          await expect(page.getByText('www.coming.com')).toHaveAttribute(
            'href',
            'www.coming.com'
          );
        ...
        test('blogs are listed in descending order by their likes', async ({
          page,
        }) => {
          ...
          await expect(blogs.nth(1)).toContainText(
            'Odyssey by Cristofer Nolan'
          );
        ...
        describe('Removing blog', () => {
          test('succeeds if user is the blog creator', async ({ page }) => {
            ...
            await expect(
              page.getByText('The Odyssey by Cristofer Nolan deleted!')
            ).toBeVisible();
